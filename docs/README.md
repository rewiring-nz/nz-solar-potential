...
| Anyone who does not read code | [How a roof estimate is made](https://rewiring-nz.github.io/nz-solar-potential/method.html) | Every decision the pipeline makes between a laser scan and the number on the map, in plain English. Public; shareable. Rebuild with `python tools/build_method_page.py method.html`. | The pipeline gains or loses a step. |
| Anyone who does not read code | [Code map](../tools/build_code_graph.sh) | Build `graphify-out/graph.html`: every file and function as a clickable graph, grouped and named, each node showing its author's own docstring. Read the header for what it is and is not good for. | The build script or the tool choice changes. |
